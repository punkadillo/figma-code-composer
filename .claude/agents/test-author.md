...
Minimal, **single-line** inline comments (**hard cap ≤80 chars**, leader→EOL excl. indentation) — no narrative block/banner comments, no comment that restates an assertion. Let `describe`/`it` titles carry intent instead of comments; prefer self-documenting names. Cuts output tokens every build. Full rule: `protocols/figma-manifest.md` § Emission discipline.
