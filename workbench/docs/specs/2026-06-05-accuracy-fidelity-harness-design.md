...
**Goal.** Compute real `visual`, `style`, and `structural` accuracy sub-scores for the Reference trial by comparing the **generated target component against the Reference oracle**, combine them with the deterministic build gates into the existing accuracy composite, write the result into each run's `results.json`, and re-render the report + dashboard so the accuracy column stops being all-null.

**Reference model (decided).** All fidelity sub-scores are **target ↔ Reference oracle**. The Figma design is **not** a reference. Rationale: a Figma node is a raster with no computed style and no DOM, so it can only feed `visual`; comparing the rendered component against the Reference implementation keeps a single coherent reference for all three sub-scores and stays fully local (both Storybooks are installed on disk — no Figma MCP).
...
|---|---|---|---|
| **visual** (pixel-diff) | Reference render | both Storybooks (Playwright screenshot) | Button, Input, Card, Alert |
| **style** (`getComputedStyle`) | Reference computed styles | both rendered DOMs over `STYLE_PROPS` | Button, Input, Card, Alert |
| **structural** (tree + props) | Reference source | both `.tsx` parsed (no render) | Button, Input, Card, Alert, **Form** |
| **gates** | — | deterministic `tsc` / `build` / `tests` already captured | all 5 |

`Form` has Reference **source** (`form.tsx`) but **no Reference story**, so it gets structural + gates but no visual/style. The rung→component→story mapping:
...
- **Build once, serve static.** Run `storybook build` for the target (`target/`) and for Reference (`ref-oracle/packages/storybook`), producing `storybook-static/` each; serve over a local static file server (no network). Static build is chosen over `storybook dev` for determinism (no HMR flake). If a static build proves too slow/fragile, fall back to `storybook dev --ci -p <port>` started as a background process.
...
- Floor case (Form, or Reference Storybook down): composite is computed from `structural` + `gates` renormalised to 1 → still a real number, never null.
...
- **Graceful degradation:** any of {Reference Storybook won't build, a story id missing, axe unavailable} degrades *that* sub-score to `unavailable` and renormalises; the run still yields structural + gates. Accuracy stops being all-null regardless of how much of the render path succeeds.
- **Honest caveat (kept in the report):** the target is `designSystem: none` / inline-Tailwind **by deliberate choice**, so target-vs-Reference `visual` and `style` will read **low** — they measure divergence from Reference's specific look, not code quality. The meaningful signals are **structural** and the **relative trend across rungs**. This caveat is surfaced in `report.md` and `analysis/01-accuracy-feasibility.md`.
...
- **P2 — render harness (visual + style vs Reference).** Stand up both Storybooks + Playwright; populate visual/style for the 4 rungs with an oracle story.
...
- **`structuralDom`** — NEW (P2): compare the **rendered DOM** trees of both Storybooks (target vs Reference). Both normalise to real HTML elements, so the tree comparison is meaningful. Available only for the 4 rungs with a Reference story.
...
The single real execution risk is **P2: whether Reference's pnpm-workspace Storybook builds/serves** in this environment (it needs `@reference/styles` built). P1 and P3-gates are independent of it. If P2 fails after reasonable effort, the harness records visual/style as `unavailable` and we ship P1+P3 — accuracy is still real (structural + gates), just without the Reference pixel/style comparison.
