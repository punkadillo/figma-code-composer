...
4. **Unbound values are flags, not invitations.** No variable binding → manifest records the raw value AND `unbound: true`. Builders MUST stop-and-flag — never invent a token or inline the raw value. *A pre-recorded `config.autonomy` policy counts as the user's answer:* when `autonomy.level == "autonomous"`, the coordinator resolves the flag from `autonomy.onUnbound` (e.g. `inline-and-flag`) and logs the decision — this is the user deciding in advance, not a builder inventing a value.
...
8. **Comment economy in generated files.** Every emitted file (component / token / icon / story / test / config) ships minimal, **single-line inline comments capped at 80 chars** (from the `//`/`#` leader to EOL, excluding indentation) — no narrative block, banner, or restating comments, no wrapping. **Exception:** concise `/** … */` JSDoc on exported components/props is allowed and encouraged (it's the public-API contract the quality docs/dx + propTypeCompleteness metrics reward). Token economy on narrative comments first, DX second. Canonical rule + required-marker exceptions: `protocols/figma-manifest.md` § Emission discipline.
