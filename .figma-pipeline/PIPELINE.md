...
8. **Comment economy in generated files.** Every emitted file (component / token / icon / story / test / config) ships minimal, **single-line comments capped at 80 chars** (from the `//`/`/*`/`#` leader to EOL, excluding indentation) — no block/banner/restating comments, no wrapping. Output tokens first (each comment is generated on every build), DX second. Canonical rule + required-marker exceptions: `protocols/figma-manifest.md` § Emission discipline.
